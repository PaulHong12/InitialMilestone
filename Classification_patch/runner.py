...
class Runner():
    ...
    def tester_only_label(self, img_path, save_res_dir):
        ...
        import csv
        with open('out_small_test.csv',mode='w',newline='') as csv_file:
            ...
            for roots,dirs,files in os.walk(img_path):
                for file in files:
                    ...
                    print(file_path,cats_top_10[1])
                    writer.writerow({'file_path':str(file_path),'label':cats_top_10[1]})
...
if __name__ == '__main__':
    ...
    # 评估
        ...
        runner.tester_only_label(config['img_path'], config['save_res_dir'])
    else:
